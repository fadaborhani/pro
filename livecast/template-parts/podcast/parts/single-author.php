...
<div class="single-author">
	...
	if( isset( $post->post_author ) ) {		
		...
		$user_posts = codeless_get_mod( 'single_podcast_author_page', false );
		$user_posts = !empty( $user_posts ) ? get_the_permalink( $user_posts ) : '';
		if( !empty( $user_posts ) ){
			$user_posts = add_query_arg( 'author-id', $post->post_author, $user_posts );
		}
		...
		$author_details .= '<div class="author_wrapper">';
			...
			// Author avatar
			$author_details .= '<div class="author_avatar">' . get_avatar( get_the_author_meta('user_email') , 150 ) . '</div>';
			// Author Bio
			$author_details .= '<div class="author_content"><h4>'.get_userdata( get_the_author_meta( 'ID' ) )->display_name.'</h4><div class="bio">' . nl2br( $user_description ) . '</div>';
				...
				if( !empty( $user_posts ) ){
				$author_details .= '<p class="author_links"><a href="'. $user_posts .'">View all podcasts by ' . $display_name . '</a>';  
				}
